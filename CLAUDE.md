...
- **Do not add fields to TypeScript interfaces without updating the Zod schema.** Zod's default behavior silently strips unknown keys during `safeParse()`. If you add a field to a TypeScript interface (e.g., `IlpChargeRule`), the code compiles and the optional-chaining checks pass gracefully — but the data is empty at runtime because Zod dropped it. The full data pipeline for validated types is: (1) TypeScript interface, (2) Zod schema, (3) template-to-policy conversion, (4) store clone functions. All four must be updated in the same commit. The sourceRefs tooltip bug (Mar 2026) was caused by updating layers 1, 3, and 4 but not the Zod schema in layer 2, resulting in tooltips that silently showed no data.
