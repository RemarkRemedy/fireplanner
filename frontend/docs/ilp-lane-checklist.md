...
- Current live cursor:
  - Fee dashboard stream:
    ...
    - `Etiqa Invest flex wealth II` is now the first post-Phase-0 family packet landed in the product-coverage lane: the published `SGD / MIP 3 years` and `SGD / MIP 5 years` corridors are executable templates backed by exact source tables, and the matching disabled picker rows are removed from that family
    - the next live cursor for product coverage is the next honest post-Phase-0 family packet after `E1`, rather than reopening audit-only or sub-fund work
