...
- Current live cursor:
  - Fee dashboard stream:
    ...
    - the next live cursor for product coverage is `T4`, with `T6` now screened as the cheaper `15..30` Tokio follow-up ahead of `T2`; keep skipping audit-only, blog, sub-fund, single-pay, and brochure-only `HSBC Wealth Focus` gaps
