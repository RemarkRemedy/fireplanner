...
- Current live cursor:
  - Fee dashboard stream:
    ...
    - targeted parser, dashboard, review-page, catalog source/meta, typecheck, and bounded `Legacy Flex Solitaire` golden-fixture economics gates are green for the widened family; the next live cursor is `A2` (`AIA Platinum Wealth Legacy` single-pay), while brochure-only `H1` remains source-blocked and `A1` still waits behind the simpler `A2` sibling plus the later no-lapse carry seam
