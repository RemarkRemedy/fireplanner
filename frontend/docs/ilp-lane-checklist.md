...
- Current live cursor:
  - Fee dashboard stream:
    ...
    - targeted parser, dashboard, review-page, catalog source/meta, typecheck, and bounded `AIA Platinum Wealth Legacy` golden-fixture economics gates are green for the widened family; the next live cursor is `A1` (`AIA Platinum Wealth Elite 2.0` mixed-lane family), while brochure-only `H1` remains source-blocked and the remaining `PR 2b` queue still needs an honest read on whether `A1` stays a bounded product-coverage packet or collapses into the no-lapse debt-carry seam
