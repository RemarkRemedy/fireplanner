...
- Current live cursor:
  - Fee dashboard stream:
    ...
    - the next live cursor for product coverage is `T6`, with `T2` still screened as the costlier `15..30` Tokio follow-up; keep skipping audit-only, blog, sub-fund, single-pay, and brochure-only `HSBC Wealth Focus` gaps
