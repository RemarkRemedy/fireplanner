...
- Current live cursor:
  - Fee dashboard stream:
    ...
    - the next live cursor for product coverage is `T2`; keep skipping audit-only, blog, sub-fund, single-pay, `A1` / `A2` until `PR 2b`, and brochure-only `HSBC Wealth Focus` gaps while `H1` stays source-blocked
