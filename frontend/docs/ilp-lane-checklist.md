...
- Current live cursor:
  - Fee dashboard stream:
    ...
    - fee-dashboard catalog surfaces no longer print raw `metadataOnlyBehaviors` lists under product names or seeded-template alerts; they now use review-needed wording so summary-absent features are not mislabeled as unsupported in the UI
    - Phase 0 catalog honesty is now landed for the product-coverage lane: the ILP picker surfaces confirmed published-but-unmodeled corridors as disabled rows on supported product cards, renders `HSBC Wealth Focus (Flexi 2)` and `Flexi 4` as disabled-only brochure-backed cards, keeps executable templates selectable, and collapses large disabled corridor families behind a show-more control
    - that Phase 0 slice is intentionally narrow:
      it is catalog visibility only through `publishedUnmodeledCorridors`, with no parser math expansion, no `templateToPolicy` changes, no runtime calculation changes, no `PRUActive LinkGuard` surfacing, and no uncertain `HSBC Wealth Harvest` / `Etiqa Invest starter` corridor exposure
    - `Etiqa Invest flex wealth II` and `Etiqa Invest Wealth Purpose` now land the first two post-Phase-0 family packets in the product-coverage lane: the published `SGD / MIP 3 years` and `SGD / MIP 5 years` corridors are executable templates backed by exact source tables, and the matching disabled picker rows are removed from both families
    - `AIA Pro Achiever 3.0` now lands the next post-Phase-0 family packet in the product-coverage lane: the published `SGD / IIP 15 years` and `SGD / IIP 20 years` corridors are executable templates backed by exact source tables, the matching disabled picker rows are removed, and the regular-account withdrawal surface now honors the published start gate from policy month `25`
    - `FWD Invest First Summit` now lands the next post-Phase-0 family packet in the product-coverage lane: the published `SGD / Premium Payment Term 11 years` through `30 years` corridors are executable templates backed by exact source tables for Booster Bonus, Loyalty Bonus, premium-reduction windows, accumulation-account premium-base caps, and surrender-charge schedules, and the matching disabled picker rows are removed from the family card
    - `FWD Invest First Max` now lands the next post-Phase-0 family packet in the product-coverage lane: the published `SGD / Premium Payment Term 11 years` through `30 years` corridors are executable templates backed by exact source tables for Booster Bonus, split Loyalty Bonus timing, Accumulation Bonus schedules, step-down initial-account charges, accumulation-account charges, top-up / recurring-single-premium charges, and exact surrender-charge schedules, and the matching disabled picker rows are removed from the family card
    - targeted parser, dashboard, review-page, and FWD-family golden economics gates are green for `FWD Invest First Max`; the broad repo-wide golden economics suite still shows unrelated locked-fixture drift on many non-FWD families, so that proof-only refresh stays out of the product-coverage lane
    - `H1` (`HSBC Wealth Focus` Flexi 2 / Flexi 4) remains source-blocked for honest executable promotion: the local source set only has full product-summary PDFs for Flexi `1 / 3 / 5`, while Flexi `2 / 4` are still brochure-only and do not publish the exact bonus / charge tables needed for parser support
    - `Tokio Marine #goAssure` now lands the next post-Phase-0 family packet in the product-coverage lane: the published `SGD / MIP 5`, `15`, `20`, and `25` corridors are executable templates backed by exact Initial Bonus tables, term-specific initial-charge slopes, exact minimum-premium floors, exact partial-withdrawal / premium-shortfall / surrender-charge schedules, and the published Loyalty / Achievement / Wellness bonus windows where applicable, and the matching disabled picker rows are removed from the family card
    - targeted parser, dashboard, review-page, catalog source/meta, and bounded `#goAssure` golden-fixture economics gates are green for the widened family; the broad repo-wide golden refresh still rewrites many unrelated locked fixtures, so only the `#goAssure` fixture set stays in-slice here
    - `TM Atlas Wealth` now lands the next Tokio family packet in the product-coverage lane: the published `SGD / MIP 5` through `25` Basic Death and Advanced Death corridors are executable templates backed by grouped initial-bonus tiers, exact term-specific surrender-charge schedules, dynamic post-MIP dividend and Initial-Units-Account access windows, and the matching disabled picker rows are removed from the family card
    - targeted parser, dashboard, review-page, catalog source/meta, and bounded Atlas golden-fixture economics gates are green for the widened family; the broad repo-wide golden refresh still rewrites many unrelated locked fixtures, so only the Atlas fixture set stays in-slice here
    - `Tokio Marine #goClassic` now lands the next Tokio family packet in the product-coverage lane: the published `SGD / MIP 5` through `25` Basic Death and Advanced Death corridors are executable templates backed by exact term-sensitive initial-bonus matrices, loyalty / Additional Bonus windows, exact surrender-charge schedules, dynamic post-MIP dividend and Initial-Units-Account access windows, and the matching disabled picker rows are removed from the family card
    - targeted parser, dashboard, review-page, catalog source/meta, and bounded `#goClassic` golden-fixture economics gates are green for the widened family; the broad repo-wide golden refresh still rewrites many unrelated locked fixtures, so only the `#goClassic` fixture set stays in-slice here
    - `Tokio Marine #goClassic Secure` now lands the next Tokio family packet in the product-coverage lane: the published `SGD / MIP 5` through `25` Basic Death and Advanced Death corridors are executable templates backed by exact term-sensitive initial-bonus matrices with honest `N/A` band pruning on longer tenors, loyalty / Additional Bonus windows, exact surrender-charge schedules, dynamic post-MIP dividend and Initial-Units-Account access windows, and the matching disabled picker rows are removed from the family card
    - targeted parser, dashboard, review-page, catalog source/meta, and bounded `#goClassic Secure` golden-fixture economics gates are green for the widened family; the broad repo-wide golden refresh still rewrites many unrelated locked fixtures, so only the `#goClassic Secure` fixture set stays in-slice here
    - `Tokio Marine #goAffluence` now lands the next Tokio family packet in the product-coverage lane: the published `SGD / MIP 15` through `30` Basic Death, Advanced Death, and Advanced Death + Life Benefit Rider corridors are executable templates backed by term-specific initial-bonus bands with honest excess-allocation layering above `100%`, exact achievement-bonus milestone maps, exact term-specific initial-charge slopes and surrender-charge schedules, dynamic loyalty / post-MIP distribution windows, and the matching disabled picker rows are removed from the family card
    - targeted parser, dashboard, review-page, catalog source/meta, typecheck, and bounded `#goAffluence` golden-fixture economics gates are green for the widened family; the broad repo-wide golden refresh still rewrites many unrelated locked fixtures, so only the `#goAffluence` fixture set stays in-slice here
    - `Tokio Marine Affluence@Future` now lands the final executable Tokio family packet in the product-coverage lane: the published `SGD / Premium Payment Term 15` through `30` Basic Death, Advanced Death, and Advanced Death + Life Benefit Rider corridors are executable templates backed by policy-year-specific initial-bonus tables with honest excess-allocation layering above `100%`, exact loyalty `Period A / B / C` timing from the rendered PDF layout, term-specific initial-charge caps and surrender-charge schedules, dynamic after-MIP withdrawal / dividend-account windows, and the matching disabled picker rows are removed from the family card
    - targeted parser, dashboard, review-page, catalog source/meta, typecheck, and bounded `Affluence@Future` golden-fixture economics gates are green for the widened family; the broad repo-wide golden refresh still rewrites many unrelated locked fixtures, so only the `Affluence@Future` fixture set stays in-slice here
    - `PR 2b` is now landed as the shared lane-metadata seam for mixed-lane product coverage: executable variants can carry first-class payment-structure, contribution-mode, premium-payment-term, policy-term, and flexi-term metadata without dashboard or seed-label drift
    - `Income Legacy Flex Solitaire` now lands the first post-`PR 2b` mixed-lane packet in the product-coverage lane: the published `SGD / Single Premium / MIP 5 years` corridor is executable with the published `4%` single-premium charge, shared manual policy-fee and insurance-cover-charge corridors, the published loyalty bonus corridor, the published Appendix 2 single-premium withdrawal / surrender schedule, and the matching disabled picker row removed from the family card
    - targeted parser, dashboard, review-page, catalog source/meta, typecheck, and bounded `Legacy Flex Solitaire` golden-fixture economics gates are green for the widened family
    - `AIA Platinum Wealth Legacy` now lands the next post-`PR 2b` mixed-lane packet in the product-coverage lane: the published `SGD / Single Pay` corridor is executable with the published `5%` single-premium charge, the shared `3%` top-up charge, the published single-pay partial-withdrawal / surrender schedule, manual illustration-backed administration-charge and insurance-risk-charge placeholders, and the matching disabled picker row removed from the family card
    - `Singlife Legacy Invest` now lands the next post-`PR 2b` mixed-lane packet in the product-coverage lane: the published SGD matrix is executable across `Single Premium / Policy Term 10`, `Single Premium / Policy Term 15`, `Premium Payment Term 3 / Policy Term 10, 15, 20`, `Premium Payment Term 5 / Policy Term 10, 15, 20`, and `Premium Payment Term 10 / Policy Term 15, 20, 25`, with exact Welcome Bonus / Special Booster / Loyalty Bonus / Maturity Bonus schedules, term-specific Administrative Charge and Appendix A withdrawal / surrender / premium-shortfall schedules, and the matching disabled picker rows removed from the family card
    - targeted parser, dashboard, review-page, catalog source/meta, typecheck, and bounded `Singlife Legacy Invest` golden-fixture economics gates are green for the widened family; `A1` (`AIA Platinum Wealth Elite 2.0`) stays rejected as a clean product-coverage packet because the remaining mixed-lane corridors collapse into the no-lapse debt-carry seam, while brochure-only `H1` remains source-blocked, so the product-coverage queue is exhausted behind real blockers unless new source files arrive or the no-lapse seam is taken up as a separate lane
  ...
  - Current-State Reconstruction override note:
    ...
    - `HSBC Wealth Focus Flexi 1/3/5` now states more explicitly that once Regular Withdrawal assumptions are already active, the current net protected premium base and current accidental-death regular-premium floor stay user-supplied by design because past top-up-first payout routing changes today’s protected base in ways this app cannot infer, and the review UI now proves those manual current-state inputs surface when needed
    - adjacent tempting candidates remain screened:
      - `HSBC Life Wealth Abundance` and `Wealth Voyage` keep their current net protected premium base / accidental-death floor inputs manual by design once Regular Withdrawal assumptions are already active because past top-up-first payout routing is not reconstructable from this app’s bounded input surface
