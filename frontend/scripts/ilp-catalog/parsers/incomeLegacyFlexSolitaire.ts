...
interface LegacyFlexRegularVariantConfig {
  id: string
...
const SINGLE_PREMIUM_CHARGE_RATE = 0.04
const TOP_UP_PREMIUM_CHARGE_RATE = 0.03
const SINGLE_PREMIUM_WITHDRAWAL_AND_SURRENDER_CHARGE_SCHEDULE = [0.19, 0.15, 0.12, 0.08, 0.04] as const

const REGULAR_VARIANT_CONFIGS: readonly LegacyFlexRegularVariantConfig[] = [
  {
...
function buildSharedFeeRules(page10: IlpCatalogSourceRef, page21: IlpCatalogSourceRef): IlpTemplateFeeRule[] {
  return [
    {
      id: 'policy-fee',
      label: 'Policy Fee',
...
function buildSharedAccounts(page1: IlpCatalogSourceRef, page2: IlpCatalogSourceRef, page12: IlpCatalogSourceRef, contributionMode: 'regular-pay' | 'single-pay'): IlpTemplateVariant['accounts'] {
  return [
    {
      id: 'premium',
      ...
      postMipFeeRate: null,
      subjectToEec: true,
      contributionRules: contributionMode === 'single-pay'
        ? [
...
function buildSharedDistributionSupport(page17: IlpCatalogSourceRef): IlpTemplateVariant['distributionSupport'] {
  return {
...
function buildRegularVariant(document: ExtractedPdfDocument, config: LegacyFlexRegularVariantConfig): IlpTemplateVariant {
  ...
  const feeRules: IlpTemplateFeeRule[] = [
    ...
    ...buildSharedFeeRules(page10, page21),
  ]
  ...
  return {
    ...
    icpMonths: 1,
    accounts: buildSharedAccounts(page1, page2, page12, 'regular-pay'),
    bonuses,
    feeRules,
    eventChargeRules,
    distributionSupport: buildSharedDistributionSupport(page17),
...
function buildSinglePremiumVariant(document: ExtractedPdfDocument): IlpTemplateVariant {
  const page1 = sourceRef(1, 'Policy description and product structure', snippetNear(document, 1, 'Legacy Flex Solitaire is a whole life investment-linked plan', 20))
  const page2 = sourceRef(2, 'Loyalty bonus, top-ups, and future premium option', snippetNear(document, 2, 'We will provide an annual loyalty bonus', 22))
  const page9 = sourceRef(9, 'Single-premium charge schedule and policy fee', snippetNear(document, 9, 'For single premium policy', 24))
  const page10 = sourceRef(10, 'Policy fee and insurance cover charge', snippetNear(document, 10, '7.2 Policy Fee', 28))
  const page11 = sourceRef(11, 'Partial withdrawal mechanics', snippetNear(document, 11, '7.7 Partial Withdrawal Charge', 24))
  const page12 = sourceRef(12, 'Subscription of premium and top-up units', snippetNear(document, 12, '100% of your single premium less any premium charge', 22))
  const page17 = sourceRef(17, 'Declaration and Reinvesting of Distributions', snippetNear(document, 17, 'Declaration and Reinvesting of Distributions', 18))
  const page21 = sourceRef(21, 'Appendix 1 insurance cover charge rates', snippetNear(document, 21, 'Appendix 1', 20))
  const page25 = sourceRef(25, 'Appendix 2 surrender and partial withdrawal charges', snippetNear(document, 25, 'Single Premium', 24))
  ...
  const feeRules: IlpTemplateFeeRule[] = [
    {
      ...
      rate: SINGLE_PREMIUM_CHARGE_RATE,
      amount: 0,
      appliesTo: ['premium'],
      activeWindow: 'policy-term',
      notes: [
        'Models the published 4% premium charge deducted from the accepted single premium before units are purchased.',
      ],
      sourceRefs: [page9, page12],
    },
    ...buildSharedFeeRules(page10, page21),
  ...
  const eventChargeRules: IlpTemplateEventChargeRule[] = [
    ...
    },
    {
      ...
      rate: 0,
      amount: 0,
      rateSchedule: buildRateSchedule(SINGLE_PREMIUM_WITHDRAWAL_AND_SURRENDER_CHARGE_SCHEDULE),
      activeWindow: 'during-mip',
      allocation: 'equal-split',
      notes: [
        'Models the published Appendix 2 charge factor on partial withdrawals from the premium account during the 5-year single-premium minimum investment period.',
        'Top-up-account withdrawals after 12 months are charge-free, but first-12-month top-up-account withdrawal charges remain informational only in V1.',
        'Qualifying Withdrawal Access Option withdrawals can be represented in V1 by setting chargeWaived on the premium-account partial-withdrawal event.',
        'Withdrawal Access Option timing, 5%-of-prevailing-premium-account-value limits, and once-per-policy-year administration remain manual in V1.',
      ],
  ...
  const bonuses: IlpTemplateBonus[] = [
    {
      ...
      suspensionRules: [{ trigger: 'partial-withdrawal', suspensionMonths: 12 }],
      notes: [
        'Applied annually from the end of the 5-year MIP on the premium-account policy value.',
        'Qualifying Withdrawal Access Option withdrawals and top-up-account withdrawals can be represented in V1 by setting bonusSuspensionWaived on the recorded partial-withdrawal event.',
      ],
      sourceRefs: [page2],
    },
  ...
  return {
    ...
    icpMonths: 1,
    accounts: buildSharedAccounts(page1, page2, page12, 'single-pay'),
    bonuses,
    feeRules,
    eventChargeRules,
    distributionSupport: buildSharedDistributionSupport(page17),
    eecTable: [...SINGLE_PREMIUM_WITHDRAWAL_AND_SURRENDER_CHARGE_SCHEDULE],
    warnings: [
...
export function parseIncomeLegacyFlexSolitaire({ document, sourceChecksumSha256 }: ParseContext): IlpCatalogProduct {
  return {
    ...
    warnings: [
      'Legacy Flex Solitaire (VA3S / VA3R) is cataloged as supported in V1 for the regular-premium 5-year and 10-year corridors plus the 5-year single-premium corridor. The current parser models the premium-year regular premium charges, the 4% single-premium charge, manual-input policy-fee and Appendix 1 insurance-cover-charge corridors, the current-state death and terminal-illness benefit estimate as the higher of adjusted sum assured or policy value via a manual current adjusted sum assured input, the current admitted-state TI payable amount through the published full-termination TI corridor after manual claim-amount entry, an admitted-and-settled TI claim as a current policy-termination state, the published Loyalty Bonus rate with the supported partial-withdrawal suspension subset, the top-up premium charge, regular-corridor premium-holiday charge, premium-account Appendix 2 withdrawal / surrender charges, and the published reinvest-only distribution baseline. Automatic adjusted-sum-assured updates, No Lapse Guarantee, retirement-option payout elections, top-up-account first-12-month charge timing, and broader claim-side settlement / continuation mechanics remain outside the current engine.',
    ...
    archived: false,
    variants: [
      ...REGULAR_VARIANT_CONFIGS.map((config) => buildRegularVariant(document, config)),
