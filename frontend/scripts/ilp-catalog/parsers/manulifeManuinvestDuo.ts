...
function buildVariant(document: ExtractedPdfDocument, config: ManuInvestDuoVariantConfig): IlpTemplateVariant {
  ...
  return {
    ...
    warnings: [
      `ManuInvest Duo is cataloged as a supported V1 corridor for the ${config.mipLength}-year MIP. The parser captures the published 5.00% / 1.00% administration-charge path, the protected-base cost-of-insurance formula after you enter insured-life details and current sum insured, the current-state death-benefit estimate from that same current sum insured, the prevailing 0% top-up charge, the MIP partial-withdrawal charge schedule, the MIP full-surrender charge schedule, and the reinvest-default distribution-mode assumption surface.`,
      ...
      'Welcome Bonus, Loyalty Bonus, withdrawal-flexibility fee handling, and fund-level management charges remain informational only.',
