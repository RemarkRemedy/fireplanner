...
describe('parseTokioMarineHarvestBuilderAtFuture', () => {
  it('builds a valid supported Harvest Builder@Future product from the source PDF', async () => {
    ...
    expect(product.metadataOnlyBehaviors).toContain('tokio-harvest-builder-atfuture-advanced-death-benefit-payout-handling')
    expect(product.metadataOnlyBehaviors).toContain('tokio-harvest-builder-atfuture-credit-card-charge')
    expect(product.metadataOnlyBehaviors).toContain('tokio-harvest-builder-atfuture-life-replacement-administration')
    ...
    expect(product.metadataOnlyBehaviors).toContain('tokio-harvest-builder-atfuture-rider-premium-deduction-handling')
    expect(product.metadataOnlyBehaviors).not.toContain('tokio-harvest-builder-atfuture-advanced-death-benefit-selection')
    expect(product.metadataOnlyBehaviors).not.toContain('tokio-harvest-builder-atfuture-life-benefit-rider')
    ...
    expect(riderVariant?.sourceRefs.some((ref) => ref.page === 14)).toBe(true)
  }, 30_000)
})
