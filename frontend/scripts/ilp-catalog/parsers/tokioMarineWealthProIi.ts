...
function buildVariant(
  ...
  if (isAdvancedDeath && page10 != null && page17 != null) {
    feeRules.push(buildTokioMpcFeeRule(page2, page10, page17, hasLifeBenefitRider))
  }
  ...
  return {
    ...
    unsupportedItems: [
      ...
      'Involuntary unemployment and hospitalisation benefit approval timing, proof, exclusions, first-assured coverage, and Tokio’s discretionary variation of benefit grant counts remain informational only beyond the modeled explicit chargeWaived plus optional shared chargeWaiverGrantId event path.',
    ],
...
export function parseTokioMarineWealthProIi(context: ParseContext): IlpCatalogProduct {
  return {
    ...
    warnings: [
      ...
      'The Advanced Death and Advanced Death with Life Benefit Rider variants both model the published first-three-policy-years Monthly Protection Charge accrual and policy-year-4 lump-sum catch-up before normal monthly-in-advance deductions resume.',
    ],
    archived: false,
    variants: [
