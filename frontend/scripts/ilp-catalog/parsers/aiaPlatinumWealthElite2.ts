...
export function parseAiaPlatinumWealthElite2(context: ParseContext) {
  const product = buildAiaPlatinumWealthProduct(context, {
    ...
    productWarning: 'The parser captures premium-year regular premium charges, the 3% top-up premium charge with blocking in months where regular premiums are not paid when due, the premium-holiday charge schedule, the regular-premium withdrawal / surrender charge schedules, the first-layer administration charge via issue-age and initial insured-amount inputs, a manual-input insurance-risk-charge placeholder sourced from the policy illustration, a static-assumption Vitality Fund Boost schedule for the regular-pay corridor, the current-state death benefit as the higher of current insured amount or policy value via a manual current insured amount input that remains user-supplied by design in this app, and the current terminal-illness snapshot plus the current residual death-benefit estimate after a TI claim today from the same supported acceleration corridor after a manual remaining aggregate TI cap is supplied, while the single-pay corridor, premium-term extension, Free Legacy Cover waivers, no-lapse mechanics, bequest elections, and terminal-illness claim exclusions / settlement workflow remain informational only beyond the modeled current ordinary death, terminal-illness, and residual-after-TI snapshot surface.',
    ...
    unsupportedItems: [
      ...
      'Insurance Risk Charge is modeled only as a manual-input annualized placeholder because the applicable insurer illustration rate depends on underwriting and Free Legacy Cover state. Free Legacy Cover and No Lapse Privilege debt accrual remain informational only.',
      ...
      'The current death benefit keeps a manual current insured amount input because withdrawals, Income Withdrawal Privilege usage, and claim-side reductions change the live insured amount in ways this app cannot observe; that field is manual by design in V1.',
