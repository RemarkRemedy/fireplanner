...
export function parseAiaPlatinumWealthElite2(context: ParseContext) {
  const product = buildAiaPlatinumWealthProduct(context, {
    ...
    catalogWarningLabel: 'supported V1 product for the regular-pay 5 to 10-year corridors plus the single-pay corridor',
    regularPremiumTermYears: [5, 6, 7, 8, 9, 10],
    ...
    regularPremiumChargeSchedule: [
      ...
      { startPolicyYear: 6, endPolicyYear: null, rate: 0.03 },
    ],
    ...
    regularPartialWithdrawalChargeFinalTierContinuesOnward: true,
    modeledEconomics: [
    ...
    productWarning: 'The parser captures premium-year regular premium charges across the regular-pay 5 to 10-year corridors, the 5% single-premium charge on the single-pay corridor, the 3% top-up premium charge with blocking in months where regular premiums are not paid when due, the premium-holiday charge schedule on the regular-pay corridor, the published regular-premium and single-premium withdrawal / surrender charge schedules, the first-layer administration charge via issue-age and initial insured-amount inputs with no-lapse fixed-charge carry during the published privilege window, a manual-input insurance-risk-charge placeholder sourced from the policy illustration with the same fixed-charge carry seam, a static-assumption Vitality Fund Boost schedule for both the regular-pay and single-pay corridors, the current-state death benefit corridor via manual current insured amount, current amount owing, and current No Lapse Privilege mode inputs that remain user-supplied by design in this app, and the current terminal-illness snapshot plus the current residual death-benefit estimate after a TI claim today from the same supported acceleration corridor after manual current insured amount, current amount owing, current No Lapse Privilege mode, and remaining aggregate TI cap inputs are supplied, while Free Legacy Cover waivers, no-lapse activation history, bequest elections, and terminal-illness claim exclusions / settlement workflow remain informational only beyond the modeled current ordinary death, terminal-illness, and residual-after-TI snapshot surface.',
    unsupportedItems: [
      'Administration charge is modeled for the first issue-date insured-amount layer only. Change-of-insured layering and new-layer charge resets remain informational only.',
      'Insurance Risk Charge is modeled only as a manual-input annualized placeholder because the applicable insurer illustration rate depends on underwriting and Free Legacy Cover state. Free Legacy Cover, no-lapse activation history, and non-manual charge indebtedness remain informational only.',
  ...
  const vitalityNotes = [
    ...
    'This modeled bonus applies to the supported regular-pay corridors only. The single-pay corridor uses its own published issue-date and anniversary Vitality Fund Boost schedule.',
  ]
  ...
  product.variants = [
    ...product.variants.map((variant) => ({
      ...
      bonuses: [...variant.bonuses, vitalityBonus],
    })),
    buildSinglePayVariant(context),
  ]
