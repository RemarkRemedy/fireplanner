...
function buildVariant(document: ExtractedPdfDocument, mode: FlexMode | 'twenty'): IlpTemplateVariant {
  ...
  return {
    ...
    warnings: [
      ...
      'Premium-Free Period gating, premium-shortfall charge, and insurance charge remain informational only in V1.',
...
function buildProduct(context: ParseContext, kind: ProductKind): IlpCatalogProduct {
  ...
  return {
    ...
    warnings: [
      `${productName} is currently modeled as a partial product in V1. The parser captures the bounded accumulation mechanics and reinvest-default distribution support, but Premium-Free Period gating, premium shortfall charge, insurance charge, and distribution-paying fund thresholds and withdrawal consequences remain outside the calculator.`,
    ],
    archived: false,
    variants: [
