...
describe('parseTokioMarineWealthFlexiLink312', () => {
  it('builds a valid supported Wealth Flexi-Link 3.12 product from the source PDF', async () => {
    ...
    expect(product.metadataOnlyBehaviors).toContain('tokio-wealth-flexi-link-3-12-involuntary-unemployment-benefit-administration')
    ...
    expect(basicVariant?.warnings.some((warning) => warning.includes('after-first-three-policy-years regular-premium variation start gate'))).toBe(true)
    ...
    expect(basicVariant?.bonuses.find((bonus) => bonus.id === 'power-up-bonus-policy-year-10')?.qualificationRules).toEqual([
      { trigger: 'partial-withdrawal', accountIds: ['accumulation'], disqualifyIfAnyInLookbackMonths: 12 },
    ])
    ...
    expect(basicVariant?.scheduledPayoutSupport).toEqual({
      ...
      sourceRefs: [expect.objectContaining({ page: 8 })],
    })
