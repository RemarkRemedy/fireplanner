...
import {
  ILP_VERTICAL_BAR_RADIUS,
  ILP_VERTICAL_NEGATIVE_BAR_RADIUS,
} from "./chartBarRadii";
...
const DEFAULT_FEE_CATEGORIES = [
  {
    ...
    label: "Account Mgt",
    description:
      "Annual percentage of account value (the ILP wrapper cost). IUA and AUA rates may differ, and post-MIP rates may apply after the minimum investment period.",
  },
  {
    key: "additionalCharges",
    label: "Additional",
    description:
      "Premium-based charges, fixed annual policy fees, cumulative-premium charges, and other recurring charge rules that are not tied to specific events.",
  },
  {
    key: "assuranceCharges",
    label: "Assurance/COI",
    description:
      "Cost-of-insurance charges for death, terminal illness, TPD, and accidental death coverage. These increase with age and are deducted from account value.",
  },
  {
    key: "eventCharges",
    label: "Event",
    description:
      "Charges triggered by specific actions: partial withdrawals, premium holidays, premium reductions, or top-ups. May include early exit charges on event amounts.",
  },
  {
    key: "implicitFundFee",
    label: "Fund Mgt (OCF)",
    description:
      "Ongoing fund management charges deducted inside the fund NAV. Not charged as a visible line item, but reduces your investment returns every year. Based on the weighted OCF of your selected funds.",
  },
] as const;
...
function deriveFeeColumnInfo(policy: IlpPolicyInput) {
  ...
  const assuranceRules = allRules.filter(
    (r) => r.basis === "assurance-sum-at-risk",
  );
  // Everything that's not account-value or assurance feeds into "additional charges"
  const additionalRules = allRules.filter(
    (r) => r.basis !== "account-value" && r.basis !== "assurance-sum-at-risk",
  );
  const eventRules = policy.eventChargeRules ?? [];

  const additionalLabel =
    additionalRules.length === 1
...
const BONUS_CONDITION_TOOLTIPS = {
  premiumHoliday:
    "A premium holiday is a period when you stop paying regular premiums for a while under the policy rules. Some products reduce or suspend bonuses during that period.",
  partialWithdrawal:
    "A partial withdrawal means taking out part of the policy value without fully surrendering the policy. That can affect bonus eligibility, surrender charges, or remaining value floors.",
  premiumReduction:
    "A premium reduction means lowering the regular premium you commit to pay. Some products treat that as a change that can reduce, suspend, or claw back bonus support.",
} as const;
...
export function getVisibleAnnualFeeCategoryKeys(
  ...
  return DEFAULT_FEE_CATEGORIES.filter(
    (category) => includeOcf || category.key !== "implicitFundFee",
  )
    .filter((category) =>
      data.some((row) => Math.abs(row[category.key]) > 0.005),
    )
...
export function FeeBreakdownSection({
  ...
  const breakdown = useMemo(
    () => buildFeeBreakdown(projection, policy.funds, policy),
    [projection, policy.funds, policy],
  );
  ...
  const inceptionTotal = breakdown.inceptionCharges.reduce(
    (s, c) => s + c.amount,
    0,
  );
  ...
  const visibleBreakdownRows = tableExpanded
    ? breakdown.rows
    : breakdown.rows.slice(0, previewRowCount);
  const hiddenRowCount = Math.max(0, breakdown.rows.length - previewRowCount);
  const displayMoney = (value: number, year: number) =>
    useRealValues ? value / Math.pow(1 + inflationRate, year) : value;

  const stackedBarData = useMemo(() => {
    const discount = (value: number, year: number) =>
      useRealValues ? value / Math.pow(1 + inflationRate, year) : value;
    ...
    return [
      ...
      ...breakdown.rows.map((row) => ({
        ...
        implicitFundFee: includeOcf
          ? discount(row.implicitFundFee, row.year)
          : 0,
  ...
  const visibleExplanationCategories = useMemo(
    () =>
      DEFAULT_FEE_CATEGORIES.filter((category) =>
        visibleAnnualFeeCategoryKeys.includes(category.key),
      ),
  ...
  const scenarioReturnAssumptions = useMemo(() => {
    const totalAllocation = policy.funds.reduce(
    ...
    return policy.funds.reduce(
      (totals, fund) => ({
        low:
          totals.low +
          (fund.allocation / totalAllocation) * fund.grossReturnLow,
        mid:
          totals.mid +
          (fund.allocation / totalAllocation) * fund.grossReturnMid,
        high:
          totals.high +
          (fund.allocation / totalAllocation) * fund.grossReturnHigh,
      }),
      { low: 0, mid: 0, high: 0 },
    );
  }, [policy.funds]);
  ...
  const cumulativeData = useMemo(() => {
    ...
    return [
      ...
      ...breakdown.rows.map((row) => {
        ...
        const grossThisYear =
          row.cumulativeGrossFees - (previousRow?.cumulativeGrossFees ?? 0);
        const bonusThisYear =
          row.cumulativeBonuses - (previousRow?.cumulativeBonuses ?? 0);
        cumGross += discount(
          grossThisYear + (includeOcf ? row.implicitFundFee : 0),
          row.year,
        );
        ...
        >
                ...
                const label2 =
                  name === "bonusCredits"
                    ...
                    : name === "additionalCharges"
                      ...
                      : (DEFAULT_FEE_CATEGORIES.find((c) => c.key === name)
                          ?.label ?? name);
                return {
                  label: label2,
                  value: formatIlpCurrency(
            ...
            formatter={(value: string) =>
              value === "bonusCredits"
                ...
                : value === "additionalCharges"
                  ? feeColumnInfo.additional.label
                  : (DEFAULT_FEE_CATEGORIES.find((c) => c.key === value)
                      ?.label ?? value)
            }
          />
          ...
          {visibleAnnualFeeCategoryKeys.includes("additionalCharges") && (
            <Bar
              ...
              radius={
                topAnnualFeeKey === "additionalCharges"
                  ? ILP_VERTICAL_BAR_RADIUS
                  : undefined
              }
            />
          ...
          {visibleAnnualFeeCategoryKeys.includes("assuranceCharges") && (
            <Bar
              ...
              radius={
                topAnnualFeeKey === "assuranceCharges"
                  ? ILP_VERTICAL_BAR_RADIUS
                  : undefined
              }
            />
          ...
          {visibleAnnualFeeCategoryKeys.includes("implicitFundFee") && (
            <Bar
              ...
              radius={
                topAnnualFeeKey === "implicitFundFee"
                  ? ILP_VERTICAL_BAR_RADIUS
                  : undefined
              }
            />
          ...
          <Bar
            ...
            stackId="fees"
            fill={colors.success}
            radius={ILP_VERTICAL_NEGATIVE_BAR_RADIUS}
          />
        </BarChart>
      </ResponsiveContainer>
    </IllustrationOnlyChartFrame>
  ...
  const renderFeeTable = (showAllRows = false) => {
    ...
    const displayedTotalContribution = breakdown.rows.reduce(
      (sum, row) => sum + displayMoney(row.contribution, row.year),
      0,
    );
    const displayedTotalAccountFee = breakdown.rows.reduce(
      (sum, row) => sum + displayMoney(row.accountFee, row.year),
      0,
    );
    const displayedTotalAdditionalCharges = breakdown.rows.reduce(
      (sum, row) => sum + displayMoney(row.additionalCharges, row.year),
      0,
    );
    const displayedTotalAssuranceCharges = breakdown.rows.reduce(
      (sum, row) => sum + displayMoney(row.assuranceCharges, row.year),
      0,
    );
    const displayedTotalEventCharges = breakdown.rows.reduce(
      (sum, row) => sum + displayMoney(row.eventCharges, row.year),
      0,
    );
    const displayedTotalImplicitFundFee = breakdown.rows.reduce(
      (sum, row) => sum + displayMoney(row.implicitFundFee, row.year),
      0,
    );
    const displayedTotalGrossFee = breakdown.rows.reduce(
      (sum, row) =>
        sum +
        displayMoney(includeOcf ? row.totalGrossFee : row.grossFee, row.year),
      0,
    );
    const displayedTotalBonusCredits = breakdown.rows.reduce(
      (sum, row) => sum + displayMoney(row.bonusCredits, row.year),
      0,
    );
    const displayedTotalWithdrawals = breakdown.rows.reduce(
      (sum, row) => sum + displayMoney(row.withdrawals, row.year),
      0,
    );
    ...
    return (
      <div
        className={cn(
          ...
          showAllRows && "max-h-[70vh]",
        )}
      >
        <table className="w-full text-sm">
          <thead className="sticky top-0 z-20 border-b bg-background">
            <tr>
              <th className="sticky left-0 z-30 border-r bg-background px-3 py-2 text-left font-medium text-muted-foreground">
                PY
              </th>
              ...
              <th className="px-2 py-2 text-right font-medium text-muted-foreground">
                <span className="inline-flex items-center gap-0.5">
                  Account Mgt
                  <FeeRuleTooltip
                    rules={feeColumnInfo.accountMgt}
              ...
              <th className="px-2 py-2 text-right font-medium text-muted-foreground">
                <span className="inline-flex items-center gap-0.5">
                  {feeColumnInfo.additional.label}
                  <FeeRuleTooltip
                    rules={feeColumnInfo.additional.rules}
                    currency={policy.currency}
                  />
                </span>
              </th>
          ...
          <tbody>
            {breakdown.inceptionCharges.length > 0 && (
              <tr className="border-b bg-amber-50/50 dark:bg-amber-950/20">
                <td className="sticky left-0 z-10 border-r bg-amber-50/50 px-3 py-2 font-medium dark:bg-amber-950/20">
                  0
                </td>
                ...
                <td className="px-2 py-2 text-right tabular-nums font-medium">
                  {formatIlpCurrency(
                    breakdown.inceptionCharges.reduce(
                      (s, c) => s + c.amount,
                      0,
                    ),
                    policy.currency,
            ...
            {rows.map((row, rowIndex) => {
              ...
              return (
                ...
                >
                  <td
                    className={cn(
                      "sticky left-0 z-10 border-r px-3 py-2 font-medium",
                      stickyRowBackground,
                    )}
                  >
                  ...
                  <td className="px-2 py-2 text-right tabular-nums">
                    {formatIlpCurrency(
                      displayMoney(row.contribution, row.year),
                      policy.currency,
                    )}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums">
                    {formatIlpCurrency(
                      displayMoney(row.accountFee, row.year),
                      policy.currency,
                    )}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums">
                    {formatIlpCurrency(
                      displayMoney(row.additionalCharges, row.year),
                      policy.currency,
                    )}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums">
                    {formatIlpCurrency(
                      displayMoney(row.assuranceCharges, row.year),
                      policy.currency,
                    )}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums">
                    {formatIlpCurrency(
                      displayMoney(row.eventCharges, row.year),
                      policy.currency,
                    )}
                  </td>
                  {includeOcf && (
                    <td className="px-2 py-2 text-right tabular-nums">
                      {formatIlpCurrency(
                        displayMoney(row.implicitFundFee, row.year),
                        policy.currency,
                      )}
                    </td>
                  )}
                  <td className="px-2 py-2 text-right tabular-nums font-medium">
                    {formatIlpCurrency(
                      displayMoney(
                        includeOcf ? row.totalGrossFee : row.grossFee,
                        row.year,
                      ),
                      policy.currency,
                    )}
                  ...
                  <td className="px-2 py-2 text-right tabular-nums text-emerald-700 dark:text-emerald-400">
                    {formatIlpCurrency(
                      displayMoney(row.bonusCredits, row.year),
                      policy.currency,
                    )}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums font-medium">
                  ...
                  <td className="px-2 py-2 text-right tabular-nums">
                    {formatIlpCurrency(
                      displayMoney(row.withdrawals, row.year),
                      policy.currency,
                    )}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums">
                    {formatIlpCurrency(
                      displayMoney(row.closingValue, row.year),
                      policy.currency,
                    )}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums">
                    {formatIlpCurrency(
                      displayMoney(row.eecCharge, row.year),
                      policy.currency,
                    )}
                  </td>
                  <td className="px-2 py-2 text-right tabular-nums">
                    {formatIlpCurrency(
                      displayMoney(row.surrenderValue, row.year),
                      policy.currency,
                    )}
                  </td>
                </tr>
            ...
            <tr className="border-t-2 bg-slate-100/80 font-semibold dark:bg-slate-800/70">
              <td className="sticky left-0 z-10 border-r bg-slate-100/80 px-3 py-2 dark:bg-slate-800/70">
                Total
              </td>
              <td className="px-2 py-2 text-right tabular-nums font-semibold">
                {formatIlpCurrency(displayedTotalContribution, policy.currency)}
              </td>
              <td className="px-2 py-2 text-right tabular-nums font-semibold">
                {formatIlpCurrency(displayedTotalAccountFee, policy.currency)}
              </td>
              <td className="px-2 py-2 text-right tabular-nums font-semibold">
                {formatIlpCurrency(
                  displayedTotalAdditionalCharges,
                  policy.currency,
                )}
              </td>
              <td className="px-2 py-2 text-right tabular-nums font-semibold">
                {formatIlpCurrency(
                  displayedTotalAssuranceCharges,
                  policy.currency,
                )}
              </td>
              <td className="px-2 py-2 text-right tabular-nums font-semibold">
                {formatIlpCurrency(displayedTotalEventCharges, policy.currency)}
              </td>
              {includeOcf && (
                <td className="px-2 py-2 text-right tabular-nums font-semibold">
                  {formatIlpCurrency(
                    displayedTotalImplicitFundFee,
                    policy.currency,
                  )}
                </td>
              )}
              <td className="px-2 py-2 text-right tabular-nums font-semibold">
                {formatIlpCurrency(displayedTotalGrossFee, policy.currency)}
              </td>
              <td className="px-2 py-2 text-right tabular-nums font-semibold text-emerald-700 dark:text-emerald-400">
                {formatIlpCurrency(displayedTotalBonusCredits, policy.currency)}
              </td>
              <td className="px-2 py-2 text-right tabular-nums font-semibold">
                {formatIlpCurrency(
                  displayedTotalGrossFee - displayedTotalBonusCredits,
                  policy.currency,
                )}
              </td>
              <td className="px-2 py-2 text-right tabular-nums font-semibold">
                {formatIlpCurrency(displayedTotalWithdrawals, policy.currency)}
              </td>
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Returns are not guaranteed, but fees are. This breakdown shows the
              projected fees under your assumptions, year by year.
            </p>
          </div>
          <div className="flex flex-col gap-3 sm:items-end">
            ...
            <Tabs
              value={scenario}
              onValueChange={(value) => setScenario(value as ReturnScenario)}
            >
              <TabsList>
            ...
            <div className="flex flex-wrap items-center gap-3 sm:justify-end">
              <div className="flex items-center gap-1.5 text-xs">
                <Checkbox
                  id="exclude-ocf"
                  checked={!includeOcf}
                  onCheckedChange={(v) => setIncludeOcf(v !== true)}
                />
                <label htmlFor="exclude-ocf" className="cursor-pointer">
              ...
              {showDollarBasisToggle ? (
                <div className="flex items-center gap-2">
                  ...
                  <Tabs
                    value={useRealValues ? "real" : "nominal"}
                    onValueChange={(value) =>
                      setUseRealValues(value === "real")
                    }
                  >
                    <TabsList>
            ...
            <div className="mb-2 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  Positive bars are fees charged. Negative bars (below zero) are
                  bonus credits returned.
                </p>
              </div>
              <Button
                type="button"
          ...
          <div>
            <div className="mb-2 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  Shows whether bonus credits meaningfully offset gross fees
          ...
          <div>
            ...
            {hasHiddenTableRows && (
              <p className="mb-3 text-xs text-muted-foreground">
                Showing the first {previewRowCount} policy years by default.
                Expand to inspect the full yearly fee path.
              </p>
            )}
            {renderFeeTable()}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {visibleExplanationCategories.map((category) => {
                const label =
                  category.key === "additionalCharges"
                    ? feeColumnInfo.additional.label
                    : category.label;
                const description =
                  category.key === "additionalCharges" &&
                  feeColumnInfo.additional.rules.length > 0
                    ? feeColumnInfo.additional.rules
                        .map((r) => r.label)
                        .join(", ")
              ...
              {hasVisibleBonusCredits && (
                <div className="rounded-md border border-emerald-200 p-3 dark:border-emerald-900">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Power-up, loyalty, allocation, and sign-up bonuses credited
                    to accounts. May be suspended or disqualified during premium
                    holidays
                    <InfoTooltip
                      text={BONUS_CONDITION_TOOLTIPS.premiumHoliday}
                    />
                    , partial withdrawals
                    <InfoTooltip
                      text={BONUS_CONDITION_TOOLTIPS.partialWithdrawal}
                    />
                    , or premium reductions
                    <InfoTooltip
                      text={BONUS_CONDITION_TOOLTIPS.premiumReduction}
                    />
                    .
                  </p>
                </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setExpandedPanel(null)}
      >
          ...
          <div className="min-h-0 flex-1 overflow-auto">
            <div className="space-y-4">
              {expandedPanel === "annual-chart" && (
                <>
                  <p className="text-sm text-muted-foreground">
                    Positive bars are fees charged. Negative bars (below zero)
                    are bonus credits returned.
                  </p>
