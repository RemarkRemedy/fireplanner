...
interface CoupleSummaryCardProps {
  data: WrappedData
  gradient: string
  direction: number
}

export function CoupleSummaryCard({ data, gradient, direction }: CoupleSummaryCardProps) {
  const perPersonFireAges = data.couple?.perPersonFireAge
  const bothFreeAge =
    perPersonFireAges != null &&
    perPersonFireAges[0] != null &&
    perPersonFireAges[1] != null
      ? Math.max(perPersonFireAges[0], perPersonFireAges[1])
      : null
