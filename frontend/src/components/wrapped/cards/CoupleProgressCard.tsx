  ...
  return (
    <WrappedCard gradient={gradient} direction={direction}>
      ...
      <motion.p variants={staggerChild} className="text-lg md:text-xl text-white/90">
        {displayPct >= 100
          ? "You've reached financial independence together!"
          : displayPct >= 75
            ? "Almost there. The finish line is in sight for both of you."
            : displayPct >= 50
              ? "You're halfway to financial freedom together."
              : displayPct >= 25
                ? "Great progress. Keep building as a team."
                : "Every journey starts with a first step together."}
      </motion.p>
    </WrappedCard>
  )
}
