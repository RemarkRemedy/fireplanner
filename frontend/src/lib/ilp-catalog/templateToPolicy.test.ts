...
describe('templateVariantToPolicySeed', () => {
  ...
  it('maps Dash PET Plus into a partial rider seed with reinvest-default distribution support', () => {
    const { manifest, products } = getIlpCatalog()
  ...
  it('maps Tokio Marine Wealth Max (II) advanced-death into a partial seed with accrued Tokio MPC inputs', () => {
    const { manifest, products } = getIlpCatalog()
    const product = products.find((entry) => entry.id === 'tokio-marine-wealth-max-ii')
    expect(product).toBeDefined()
  ...
  it('maps Tokio Marine Wealth Pro (II) advanced-death into a partial seed with accrued Tokio MPC inputs', () => {
    const { manifest, products } = getIlpCatalog()
    const product = products.find((entry) => entry.id === 'tokio-marine-wealth-pro-ii')
    expect(product).toBeDefined()
  ...
  it('maps Tokio Marine Harvest Pro advanced-death into a partial seed with accrued Tokio MPC inputs', () => {
    const { manifest, products } = getIlpCatalog()
    const product = products.find((entry) => entry.id === 'tokio-marine-harvest-pro')
    expect(product).toBeDefined()
  ...
  it('maps Tokio Marine Harvest Flexi advanced-death into a partial seed with Tokio MPC inputs', () => {
    ...
    expect(seed.catalogSource?.metadataOnlyBehaviors).toContain('tokio-harvest-flexi-dividend-payout-threshold-and-record-date-instructions')
  ...
  it('maps Tokio Marine Harvest Max advanced-death into a partial seed with accrued Tokio MPC inputs', () => {
    const { manifest, products } = getIlpCatalog()
    const product = products.find((entry) => entry.id === 'tokio-marine-harvest-max')
    expect(product).toBeDefined()
    ...
    expect(seed.catalogSource?.metadataOnlyBehaviors).toContain('tokio-harvest-max-dividend-payout-threshold-and-record-date-instructions')
    expect(seed.catalogWarnings?.some((warning) => warning.includes('first-three-policy-years accrual window'))).toBe(true)
  })

  it('maps Tokio Marine Wealth Flexi basic-death into a partial seed with split performance-bonus windows', () => {
    const { manifest, products } = getIlpCatalog()
  ...
  it('maps Tokio Marine Wealth Flexi advanced-death into a partial seed with Tokio MPC inputs', () => {
    const { manifest, products } = getIlpCatalog()
    const product = products.find((entry) => entry.id === 'tokio-marine-wealth-flexi')
    expect(product).toBeDefined()
  ...
  it('maps Tokio Marine #goAffluence into a partial seed with executable initial-charge and policy-charge rules', () => {
    ...
    expect(seed.catalogSource?.metadataOnlyBehaviors).toContain('tokio-goaffluence-dividend-payout-threshold-record-date-regular-withdrawal-and-partial-withdrawal-constraints')
    expect(seed.catalogWarnings?.some((warning) => warning.includes('manual distribution-mode assumption'))).toBe(true)
  })
