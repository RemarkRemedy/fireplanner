    ...
    },
    {
      ...
      fields: [
        ...
        { name: 'hasCpfis', label: 'Do you invest through CPFIS?', type: 'toggle',
          tooltip: 'CPF Investment Scheme lets you invest OA/SA funds in approved instruments. If enabled, CPFIS-invested amounts earn your specified return instead of the default 2.5%/4%.' },
        { name: 'cpfisOaReturn', label: 'CPFIS OA expected return (%)', type: 'percent',
          showWhen: { field: 'hasCpfis', equals: true },
          tooltip: 'Expected annual return on your CPFIS OA investments. Default CPF OA rate is 2.5%.' },
        { name: 'cpfisSaReturn', label: 'CPFIS SA expected return (%)', type: 'percent',
          showWhen: { field: 'hasCpfis', equals: true },
          tooltip: 'Expected annual return on your CPFIS SA investments. Default CPF SA rate is 4%.' },
      ],
    },
    {
      id: 'cpf-strategy',
      title: 'CPF Investment Strategy',
      fields: [
        { name: 'cpfVirtualRebalancing', label: 'Count uninvested CPF as bond allocation', type: 'toggle',
          tooltip: 'Treat uninvested CPF (earning guaranteed 2.5-4%) as part of your bond allocation. This tilts your liquid portfolio toward more equities since CPF already covers the conservative portion. On by default.' },
        { name: 'cpfVirtualRebalancingMode', label: 'When to apply', type: 'select',
          showWhen: { field: 'cpfVirtualRebalancing', equals: true },
          options: [
            { value: 'from55', label: 'From age 55 only (CPF becomes withdrawable)' },
            { value: 'always', label: 'Always (count even locked CPF)' },
          ] },
      ],
    },
  ],
  ...
  screens: [
    ...
    },
    {
      ...
      fields: [
        ...
        { name: 'mortgageOutstanding', label: 'Outstanding loan amount', type: 'currency', tooltip: 'Remaining loan principal.', showWhen: { field: 'hasMortgage', equals: true } },
        { name: 'monthlyMortgagePayment', label: 'Monthly repayment', type: 'currency', tooltip: 'Your monthly mortgage repayment amount.', showWhen: { field: 'hasMortgage', equals: true } },
        { name: 'mortgageRatePercent', label: 'Mortgage interest rate (%)', type: 'percent', validationKey: 'mortgageRatePercent', tooltip: 'Current annual interest rate on your mortgage.', showWhen: { field: 'hasMortgage', equals: true } },
        {
          name: 'mortgageEndYear',
          label: 'Loan end year',
          type: 'number',
          showWhen: { field: 'hasMortgage', equals: true },
          computeFrom: {
    ...
    },
    {
      ...
      fields: [
        ...
        // proceedsAllocationPercent hidden — with glide path + annual rebalancing, the invest/cash split
        // is overridden at next rebalance. Revisit when cash reserve bucket is separate from portfolio.
