...
export function getMirrorCopy(
  ...
  switch (insight.id) {
    case 'savings-power': {
      ...
      return {
        headline: isYoung
          ? `Every extra $500/month you save? That's ${yearsPerExtra500} fewer years of work. Not bad.`
          : `At your income, every $500/month saved moves your FIRE date forward by ~${yearsPerExtra500} years.`,
        detail: '',
      }
    }
    case 'savings-rate': {
      ...
      return {
        ...
        detail: futureValue > 0
          ? `Over ${yearsToGo} years at 5% returns, that alone grows to ${formatCurrency(futureValue)}.`
          : '',
      }
    }
    case 'cpf-runway': {
    ...
    case 'net-worth': {
      ...
      const parts: string[] = []
      if (hasProperty && propertyPercent > 0) parts.push(`${propertyPercent}% property`)
      parts.push(`${liquidPercent}% liquid`)
      if (hasCpf && cpfPercent > 0) parts.push(`${cpfPercent}% CPF`)
      return {
        headline: isYoung
