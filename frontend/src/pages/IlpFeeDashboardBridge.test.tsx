...
describe('ILP fee dashboard blog bridge', () => {
  ...
  it('shows the exit timing calculator with exit-year tradeoff metrics', () => {
    ...
    expect(screen.getByText('Withdrawable value vs added from now')).toBeInTheDocument()
    expect(screen.getByRole('img', { name: /line chart showing withdrawable value and added contributions by exit year/i })).toBeInTheDocument()
    expect(screen.getByText(/if the same money went into a low-cost etf instead/i)).toBeInTheDocument()
    expect(screen.getByRole('img', { name: /line chart showing withdrawable value and etf benchmark by exit year/i })).toBeInTheDocument()
