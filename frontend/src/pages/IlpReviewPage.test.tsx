...
describe('IlpReviewPage', () => {
  ...
  it('seeds Tokio Marine Affluence@Future 30-year corridor as a supported catalog product with split initial-bonus and loyalty windows', async () => {
    const user = userEvent.setup()
    renderIlpReviewPage()
    ...
    expect(screen.getAllByText('Affluence@Future (SGD / MIP 30)').length).toBeGreaterThan(0)
    const seededAlert = screen.getByText('Seeded from catalog template').closest('[role="alert"]')
    expect(seededAlert).not.toBeNull()
    expect(seededAlert?.textContent).toContain('Supported template')
    expect(seededAlert?.textContent).toContain('premium-payment-term family from 15 to 30 years')
    expect(getCatalogValue('Initial Charge')).toBeInTheDocument()
    expect(getCatalogValues('Policy Charge').length).toBeGreaterThan(0)
    expect(getCatalogValue('Initial Bonus (Policy Year 1)')).toBeInTheDocument()
    expect(getCatalogValue('Initial Bonus (Policy Year 1 Excess Rate Layer)')).toBeInTheDocument()
    expect(getCatalogValue('Initial Bonus (Policy Year 2)')).toBeInTheDocument()
    expect(getCatalogValue('Initial Bonus (Policy Year 2 Excess Rate Layer)')).toBeInTheDocument()
    expect(getCatalogValue('Loyalty Bonus (Period B: Policy Years 11-25)')).toBeInTheDocument()
    expect(getCatalogValue('Loyalty Bonus (Period C: Policy Years 26-30)')).toBeInTheDocument()
    expect(getCatalogValue('Partial Withdrawal Charge')).toBeInTheDocument()
  }, ILP_REVIEW_PAGE_TEST_TIMEOUT_MS)

  it('seeds Tokio Marine Affluence@Future 25-year advanced-death corridor as a supported catalog product with accrued Tokio MPC inputs', async () => {
    const user = userEvent.setup()
    renderIlpReviewPage()
    ...
    expect(screen.getAllByText('Affluence@Future (SGD / MIP 25 (Advanced Death))').length).toBeGreaterThan(0)
    const seededAlert = screen.getByText('Seeded from catalog template').closest('[role="alert"]')
    expect(seededAlert).not.toBeNull()
    expect(seededAlert?.textContent).toContain('Supported template')
    expect(seededAlert?.textContent).toContain('premium-payment-term family from 15 to 30 years')
  ...
  it('seeds Tokio Marine Affluence@Future 30-year advanced-death-life-benefit-rider corridor as a supported catalog product with policy-term Tokio MPC inputs', async () => {
    const user = userEvent.setup()
    renderIlpReviewPage()
    ...
    expect(screen.getAllByText('Affluence@Future (SGD / MIP 30 (Advanced Death Life Benefit Rider))').length).toBeGreaterThan(0)
    const seededAlert = screen.getByText('Seeded from catalog template').closest('[role="alert"]')
    expect(seededAlert).not.toBeNull()
    expect(seededAlert?.textContent).toContain('Supported template')
    expect(seededAlert?.textContent).toContain('premium-payment-term family from 15 to 30 years')
  ...
  it('shows Singlife Legacy Invest TI Benefit Today once current amount owing is filled', async () => {
    ...
    await user.click(within(dialog).getByRole('button', { name: /^sgd \/ premium payment term 10 years \/ policy term 15 yearsuse template$/i }))
    await confirmSeededPolicy(user)
  ...
  it('seeds AIA Platinum Wealth Legacy single-pay as a supported catalog product with single-premium charge warnings', async () => {
    const user = userEvent.setup()
    renderIlpReviewPage()
    ...
    expect(seededAlert).not.toBeNull()
    expect(seededAlert?.textContent).toContain('Supported template')
    expect(seededAlert?.textContent).toContain('supported V1 product for the regular-pay 5-year corridor plus the single-pay corridor')
    expect(seededAlert?.textContent).toContain('5% single-premium charge')
    expect(seededAlert?.textContent).toContain('published regular-pay and single-pay partial-withdrawal / surrender charge schedules')
    expect(seededAlert?.textContent).toContain('current-state death benefit corridor via manual current insured amount')
    expect(seededAlert?.textContent).toContain('current amount owing')
    expect(seededAlert?.textContent).toContain('current No Lapse Privilege mode')
    expect(screen.getByLabelText(/^Initial Single Premium \(Gross Lump Sum, /i)).toBeInTheDocument()
  ...
  it('seeds Singlife Legacy Invest as a supported catalog product with modeled booster, loyalty, maturity, and shortfall mechanics', async () => {
    ...
    await user.click(within(dialog).getByRole('button', { name: /sgd \/ premium payment term 10 years \/ policy term 15 years/i }))
    await confirmSeededPolicy(user)

    expect(screen.getAllByText('Singlife Legacy Invest (SGD / Premium Payment Term 10 years / Policy Term 15 years)').length).toBeGreaterThan(0)
    const seededAlert = (await screen.findByText('Seeded from catalog template')).closest('[role="alert"]')
    expect(seededAlert).not.toBeNull()
    expect(seededAlert?.textContent).toContain('Supported template')
    expect(seededAlert?.textContent).toContain('SGD / Premium Payment Term 10 years / Policy Term 15 years is cataloged as a supported V1 corridor')
    expect(seededAlert?.textContent).toContain('published 2.50% Special Booster at the end of the premium payment term')
    expect(seededAlert?.textContent).toContain('current-state death and terminal-illness benefit amount as the higher of 101% of paid premiums plus top-ups less withdrawals or account value less manual current amount owing')
  ...
  it('seeds FWD Invest First Max 30-year corridor as a supported catalog product with the long-tenor schedules', async () => {
    const user = userEvent.setup()
    renderIlpReviewPage()
    ...
    expect(within(dialog).getByText('FWD Invest First Max')).toBeInTheDocument()
    await user.click(within(dialog).getByRole('button', { name: /sgd \/ mip 30/i }))
    await confirmSeededPolicy(user)

    expect(screen.getAllByText('FWD Invest First Max (SGD / MIP 30)').length).toBeGreaterThan(0)
    const seededAlert = screen.getByText('Seeded from catalog template').closest('[role="alert"]')
    expect(seededAlert).not.toBeNull()
    expect(seededAlert?.textContent).toContain('Supported template')
    expect(seededAlert?.textContent).toContain('SGD 30-year base-layer corridor')
    expect(seededAlert?.textContent).toMatch(/Accumulation Bonus schedule on policy years 15\D*20\D*25\D*30/)
    expect(getCatalogValue('Loyalty Bonus (Policy Years 3-30)')).toBeInTheDocument()
    expect(getCatalogValue('Loyalty Bonus (Policy Year 31 Onward)')).toBeInTheDocument()
    expect(getCatalogValue('Accumulation Bonus')).toBeInTheDocument()
    expect(getCatalogValue('Initial Account Charge')).toBeInTheDocument()
    expect(getCatalogValue('Accumulation Account Charge')).toBeInTheDocument()
  }, ILP_REVIEW_PAGE_TEST_TIMEOUT_MS)
